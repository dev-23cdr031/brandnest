...
const maintenanceService: ServiceItem = {
  icon: Rocket,
  name: 'Website & Mobile App Maintenance',
  description:
    'Keep your website and mobile application secure, updated, and running smoothly throughout the year.',
  price: '₹899 / year',
  features: maintenanceIncludes,
  action: 'Get Maintenance Plan',
}
...
function OrderModal({ service, onClose }: { service: ServiceItem; onClose: () => void }) {
  ...
  return (
    <motion.div
      ...
      exit={{ opacity: 0 }}
    >
      <div aria-hidden className="fixed inset-0 bg-black/80 backdrop-blur-sm" onClick={onClose} />
      <motion.div
        role="dialog"
        aria-modal="true"
        aria-label={`Order ${service.name}`}
        className="relative w-full max-w-2xl rounded-[1.75rem] border border-white/15 bg-[#0c0c0c] p-6 shadow-[0_40px_140px_rgba(0,0,0,0.65)] sm:p-8"
      ...
      >
        <button
          ...
          onClick={onClose}
          aria-label="Close"
          className="absolute right-5 top-5 z-10 flex h-9 w-9 items-center justify-center rounded-full border border-white/10 bg-white/[0.06] text-white/70 transition hover:border-red-300/50 hover:text-white"
        >
          <X className="h-4 w-4" />
        </button>
        ...
        {submitted ? (
          <div className="py-14 text-center">
            <motion.div
              className="mx-auto flex h-16 w-16 items-center justify-center rounded-full bg-red-500/15 text-red-300"
              initial={{ scale: 0 }}
              animate={{ scale: 1 }}
            ...
            <h3 className="mt-6 text-2xl font-black text-white">Order Submitted!</h3>
            <p className="mx-auto mt-3 max-w-md text-white/70">
              Thank you{form.name ? `, ${form.name.split(' ')[0]}` : ''}! Your{' '}
              <span className="font-semibold text-red-200">{service.name}</span> order has been
              received. Our team will contact you at{' '}
              <span className="font-semibold text-white">{form.email || 'your email'}</span> within
              24 hours.
            </p>
            <button
              type="button"
              onClick={onClose}
              className="mt-8 rounded-full bg-gradient-to-r from-red-600 to-red-500 px-7 py-3 text-sm font-semibold text-white transition hover:from-red-500 hover:to-red-400"
            >
              Done
            </button>
          </div>
          ...
          <form onSubmit={handleSubmit}>
            <div className="flex items-center gap-3 text-red-200">
              ...
              <p className="text-xs font-semibold uppercase tracking-[0.32em]">Order Form</p>
            </div>
            <h3 className="mt-3 text-2xl font-black text-white sm:text-3xl">Order {service.name}</h3>
            <p className="mt-2 text-sm text-white/60">
              Starting at <span className="font-semibold text-red-200">{service.price}</span> — fill
              in your details and our team will confirm your order.
            </p>

            <div className="mt-7 grid gap-4 sm:grid-cols-2">
              <label className="block">
                <span className="mb-1.5 block text-xs font-semibold uppercase tracking-[0.2em] text-white/50">Full Name *</span>
                <input
                  required
                  value={form.name}
                  onChange={update('name')}
                  placeholder="John Doe"
                  className="w-full rounded-xl border border-white/10 bg-black/40 px-4 py-3 text-sm text-white outline-none transition placeholder:text-white/30 focus:border-red-300/60 focus:shadow-[0_0_30px_rgba(220,38,38,0.14)]"
                />
              </label>
              <label className="block">
                <span className="mb-1.5 block text-xs font-semibold uppercase tracking-[0.2em] text-white/50">Company / Business Name</span>
                <input
                  value={form.company}
              ...
              <label className="block">
                <span className="mb-1.5 block text-xs font-semibold uppercase tracking-[0.2em] text-white/50">Email Address *</span>
                <input
                  required
                  type="email"
                  value={form.email}
              ...
              <label className="block">
                <span className="mb-1.5 block text-xs font-semibold uppercase tracking-[0.2em] text-white/50">Phone Number *</span>
                <input
                  required
                  type="tel"
                  value={form.phone}
              ...
              <label className="block">
                <span className="mb-1.5 block text-xs font-semibold uppercase tracking-[0.2em] text-white/50">Service</span>
                <div className="w-full cursor-not-allowed rounded-xl border border-red-300/30 bg-red-500/10 px-4 py-3 text-sm font-semibold text-red-100">
                  {service.name} — {service.price}
                </div>
              </label>
              <label className="block">
                <span className="mb-1.5 block text-xs font-semibold uppercase tracking-[0.2em] text-white/50">Preferred Budget</span>
                <input
                  value={form.budget}
                  onChange={update('budget')}
                  placeholder={`e.g. ${service.price}`}
                  className="w-full rounded-xl border border-white/10 bg-black/40 px-4 py-3 text-sm text-white outline-none transition placeholder:text-white/30 focus:border-red-300/60 focus:shadow-[0_0_30px_rgba(220,38,38,0.14)]"
                />
              </label>
              <label className="block sm:col-span-2">
                <span className="mb-1.5 block text-xs font-semibold uppercase tracking-[0.2em] text-white/50">Project Requirements / Details *</span>
                <textarea
                  required
                  rows={5}
                  value={form.details}
                  onChange={update('details')}
                  placeholder="Tell us about your project — pages, features, design preferences, references..."
                  className="w-full resize-none rounded-xl border border-white/10 bg-black/40 px-4 py-3 text-sm text-white outline-none transition placeholder:text-white/30 focus:border-red-300/60 focus:shadow-[0_0_30px_rgba(220,38,38,0.14)]"
                />
              </label>
              <button
                type="submit"
                disabled={loading}
                className="inline-flex items-center justify-center gap-2 rounded-full bg-gradient-to-r from-red-600 to-red-500 px-7 py-3.5 font-semibold text-white shadow-[0_20px_65px_rgba(220,38,38,0.35)] transition hover:scale-[1.02] hover:from-red-500 hover:to-red-400 disabled:opacity-60 sm:col-span-2"
              >
          ...
          >
            <article className="rounded-[2rem] border border-red-500/20 bg-gradient-to-br from-red-600/20 via-white/5 to-black p-8 shadow-[0_25px_90px_rgba(220,38,38,0.15)] backdrop-blur-xl">
              ...
              <div className="mt-7 flex items-center justify-between gap-4 rounded-2xl border border-white/10 bg-black/30 p-5">
                ...
                <button
                  type="button"
                  onClick={() => setOrderService(maintenanceService)}
